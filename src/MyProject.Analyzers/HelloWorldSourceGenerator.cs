...
using MyProject.Analyzers.CovariantReturns;
...
public sealed class HelloWorldSourceGenerator : IIncrementalGenerator
{
    ...
    { 
        ...
        context.RegisterSourceOutput(partialsToGenerate, (spc, templateModel) =>
        {
            spc.AddSource(templateModel.FileName, SourceText.From(templateModel.Source, Encoding.UTF8));
            // templateModel = templateModel with
            // {
            //     TypeParameters = templateModel.TypeParameters.Append(new TypeParameterModel { Name = "T", Constraints = ImmutableEquatableArray<string>.Empty }).ToImmutableEquatableArray()
            // };
